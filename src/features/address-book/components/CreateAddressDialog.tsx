 ...
 * "Cascading renders" occur when a component renders, and then an Effect immediately triggers 
 * another render by calling setState. React warns about this because it forces double-rendering 
 * for every data update, which hurts performance and can lead to complex bugs.
 * 
 * The Fix:
 * Instead of using an Effect to "sync" state, we use a 'key' on the form component. 
 * When the fetched data arrives or the ID changes, the key changes, causing React to 
 * discard the old form and initialize a new one with the fresh data directly in useState.
 */
...
function AddressForm({ initialValues, isFetching, onSubmit, onCancel, isLoading, isEdit }: AddressFormProps) {
