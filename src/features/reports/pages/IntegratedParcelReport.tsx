...
import {
  ClipboardList, DollarSign, Users, Search
} from 'lucide-react';
import { DataTable } from '@/components/common/DataTable';
import { ExportMenu } from '@/components/common/ExportMenu';
import { StatCard } from '@/components/common/StatCard';
import { ADMIN_INTEGRATED_PARCEL_COLUMNS } from '../constants';
import {
  useIntegratedParcelReport,
  useExportIntegratedParcelReport
...
export default function IntegratedParcelReportPage() {
  const { role } = useAppSelector((state) => state.auth);
  const isAdmin = role === "admin";

  const [dateRange, setDateRange] = useLocalStorage<DateFilterValue>('integrated_parcel_date_range', {
    type: 'custom',
    from: undefined,
    to: undefined,
    label: 'All Time',
  ...
  const [page, setPage] = useLocalStorage<number>('integrated_parcel_page', 1);
  ...
  const [selectedCustomer, setSelectedCustomer] = useLocalStorage<string>('integrated_parcel_customer', '');
        ...
        <div className="flex flex-col gap-3 p-4 border-b border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-t-lg print:hidden">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-base font-bold text-gray-800 dark:text-zinc-200 my-0">
              Custom Integrated Courier Parcel Report
            </h1>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto flex-1 sm:flex-1">
              ...
              <ExportMenu
                ...
                onExport={(format) => exportMutation.mutate({ start_date: dateRange.from, end_date: dateRange.to, search: search || undefined, format })}
              />
            </div>
          </div>
        </div>
